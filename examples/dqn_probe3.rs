...
fn main() {
    ...
    let logger = CsvLogger::new(
        PathBuf::from("logs/dqn_probe3/dqn_probe3.csv"),
        false,
    );
    ...
    let mut trainer: OfflineTrainer<Adam<LibTorch>, Autodiff<LibTorch>, usize, usize> =
        OfflineTrainer::new(
            offline_params,
            Box::new(env),
            Box::new(ProbeEnvDiscountingTest::default()),
            Box::new(agent),
            buffer,
            Box::new(logger),
            None,

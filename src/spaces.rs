...
impl SpaceSample {
    pub fn to_tensor<B: Backend>(&self) -> Tensor<B, 1> {
        ...
        match self {
            SpaceSample::Discrete { space, idx } => Tensor::one_hot(*idx as usize, space.size(), &Default::default()),
...
impl Space {
    pub fn sample(&self) -> SpaceSample {
        ...
        match self {
            Space::Discrete { size } => {
                SpaceSample::Discrete { space: self.clone(), idx: rng.gen_range(0..*size) as i32}
            },
            Space::Continuous { lows, highs } => {
                ...
                SpaceSample::Continuous{space: self.clone(), data: random_floats}
            },
        }
    }
